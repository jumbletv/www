...
export function getBlogPostsBySlug(slug?: string): BlogPost[] {
  return BlogPosts.map((blogPost) => {
    ...
    const finalRelated: BlogPost[] = relatedPosts.map((related) => {
      const relatedAuthor: Author = Authors.find((author) => author["_id"] === related.author)
      const relatedTags: Tag[] = Tags.filter((tag) => related.tags.includes(tag["_id"]))
      const relatedType: Type = Types.find((type) => type["_id"] === related["type"])
